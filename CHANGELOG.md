...
- `BotClient` trait at `ferobot::client::BotClient`. `Bot` now stores `Arc<dyn BotClient>` instead of a raw `reqwest::Client`.
...
- `Updater` struct at `ferobot::Updater` - glues `Bot` and `Dispatcher` into a single runner.
